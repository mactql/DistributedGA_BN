...
import Experiences.ParallelSSSerialGAParallelScore._
...
object ParallelSSSerialGAParallelScore {
	var sampleName = ""
	...
	def run(): Unit = {
		...
		val ga: ParallelSSSerialGAParallelScore = new ParallelSSSerialGAParallelScore()
		ga.run()
	}
}

class ParallelSSSerialGAParallelScore extends java.io.Serializable {
	//用来判断是否连续30次都没有进步
	...
	def run(): Unit = {
		...
		//创建sparkContext
		val conf = new SparkConf().setAppName("ParallelSSSerialGAParallelScore")
				.setMaster("yarn")
