...
import Experiences.ParallelSSSerialGASerialScore._
import Config.RedisConfig
import Experiences.SingleGA.numOfPopulation
import Models.BNStructure
import Models.ScoreModels._
import Models.SuperStructure.getSSWithMutualInfo
import Operations.GAOperations.{getEliteIndividual, initPopulationAllWithRemoveCycleAndSS, replaceLowestWithElite, singlePointMutation, singlePointMutationAll, singlePointMutationWithSS, tournamentSelection, tournamentSelectionAndUniformCrossover, uniformCrossoverAll}
...
object ParallelSSSerialGASerialScore {
	var sampleName = ""
	...
	def run(): Unit = {
		...
		(0 until testLoop).foreach(a=>{
			val ga: ParallelSSSerialGASerialScore = new ParallelSSSerialGASerialScore()
			ga.run()
		})
		writer.close()
...
class ParallelSSSerialGASerialScore extends java.io.Serializable{
	//用来判断是否连续30次都没有进步
	...
	def run(): Unit = {
		...
		//创建sparkContext
		val conf = new SparkConf().setAppName("ParallelSSSerialGASerialScore")
				.setMaster("yarn")
				.setSparkHome(SPARK_JARS_HOME)
		val sc = new SparkSession.Builder().config(conf).getOrCreate().sparkContext

		//读取输入数据RDD，最小分区数为48
		val textfile: RDD[Array[String]] = sc.textFile(inputPath, 4).cache().map(_.split(",")).cache()
		...
		val BNMatrixPopulation: Array[DenseMatrix[Int]] = initPopulationAllWithRemoveCycleAndSS(numOfPopulation * 2, copyNumOfAttributes, sc, broadSS).collect()
		...
		//对BN结构种群进行评分计算
		var BNStructurePopulationArray:Array[BNStructure] = score.calculateScoreSerialWithRedis(BNMatrixPopulation,textfileArray,broadValueTpye.value,scoreJedisPipeline)
...
//			val broadPopulation: Broadcast[Array[BNStructure]] = sc.broadcast(BNStructurePopulationArray)
			...
			//BIC评分
			BNStructurePopulationArray = score.calculateScoreSerialWithRedis(BNStructurePopulationArray.map(_.structure),textfileArray,broadValueTpye.value,scoreJedisPipeline)
